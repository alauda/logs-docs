...
This guide explains how to configure the ACP Log Storage with ClickHouse plugin to use S3 for log data storage.
...
When installing **Alauda Container Platform Log Storage with Clickhouse**, set **Storage Policy** to **Separation of storage and compute**.

Keep **Cold and Hot Separation** disabled if you want to use the S3-only mode.
...
| **Cold and Hot Separation** | Keep this switch disabled for the S3-only mode. |

## Enable Cold and Hot Separation

If you want ClickHouse to keep active data locally and transfer cold data to S3, enable **Cold and Hot Separation** during installation.

After the switch is enabled, configure:

| Field | Description |
| --- | --- |
| **S3 for Cold Data** | Select the S3 Secret used for cold data storage. |
| **Retention Time for Cold Data** | Configure the retention period for each log type before it is transferred to cold storage. |
...
- The storage policy type and the cold/hot separation switch cannot be changed after deployment.
