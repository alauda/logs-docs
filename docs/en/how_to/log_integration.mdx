...
:::warning ACP 4.4 note
This guide covers the **Alauda Container Platform Log Storage for Elasticsearch** (legacy) plugin only. It uses the chart install-form field `elasticsearch.address` and the paired `basicAuthSecretName`, which are Elasticsearch-plugin conventions retained from earlier releases.

If you are on the recommended ACP 4.4 backends (ClickHouse or OpenSearch), external interfacing is the **default** install path, not an opt-in. Every storage cluster reaches an external ClickHouse or OpenSearch and an external Kafka through the two `platform-default-*` connection Secrets. Follow [Installation](../install/index.mdx) and skip this guide.
:::
