...
:::info
On Alauda OS nodes, local storage paths are under `/var/cpaas`, for example `/var/cpaas/data/clickhouse` or `/var/cpaas/data/elasticsearch`. These nodes are immutable and `/cpaas`, the layout of the traditional operating system, is not writable there. Plan the infra nodes so that the `/var/cpaas` path is backed by a persistent disk that survives node re-provisioning, and expect `pv.spec.local.path` on those nodes to reference the `/var/cpaas` path instead of the `/cpaas` paths shown in the examples below. For the per-plugin parameters, see [Installation](../install_log.mdx).
:::
