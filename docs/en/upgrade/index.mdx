...
3. You have separately prepared the target storage and message queue used by this upgrade. The upgrade does not reuse the storage or Kafka that ship with the Elasticsearch plugin, so you provide:
   ...
   - The target storage and Kafka connection details prepared as described in [Environment Preparation](../prepare/index.mdx).
...
The operator does not create the external OpenSearch/ClickHouse cluster or the new Kafka service. Create them as described in [Environment Preparation](../prepare/index.mdx), then provide the connection details through the Secrets in Step 1.
