...
The legacy Alauda Container Platform Log Storage for Elasticsearch values below are the current deployment baseline, not target settings. They are `chart-alauda-log-center` defaults and can differ per site; check the deployed values before you plan:

| Legacy Alauda Container Platform Log Storage for Elasticsearch chart setting | Typical baseline | What to prepare for the target |
| --- | --- | --- |
