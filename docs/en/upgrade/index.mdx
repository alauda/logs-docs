...
| 2 | Workload cluster | If history is needed, create `LegacyESMigration` | `PrecaptureReady` (or `Succeeded` on an existing run) |
...
| 4 | Workload cluster | Observe the same `LegacyESMigration` | `Phase=Succeeded` and target queries pass |
...
### Step 2: Prepare the historical data migration (optional)
...
### Step 4: Complete and verify the historical migration (optional)
