...
So , before installing the log storage plugin , you need to plan the cluster and node where the log storage component will be installed.
  ...
  - Separate log storage by scope. Deploy log storage in the `global` cluster for the `global` cluster's own logs, so that the `global` cluster collects, stores, and queries its own logs independently. Centralize application logs from workload clusters into a dedicated log storage workload cluster rather than into the `global` cluster, so that a `global` cluster failure does not disrupt log-based issue resolution for applications, and application log load does not affect the management cluster.

  - If the `global` cluster is deployed with disaster recovery, do not send workload cluster logs to either the primary or the standby `global` cluster. Disaster recovery synchronizes only the `global` cluster's etcd data and excludes namespaces such as `cpaas-system`, so log data is not replicated to the standby cluster. Each `global` cluster stores and queries only its own logs and never uses the other cluster's log storage; otherwise application logs become unavailable after a failover. For the synchronization scope, see <ExternalSiteLink name="acp" href="/install/global_dr.html" children="Global Cluster Disaster Recovery" />.
...
When you install a logging plugin on Alauda OS nodes, replace the `/cpaas` prefix with `/var/cpaas` in every host path parameter:

| Plugin | Parameter | Path on Alauda OS |
| --- | --- | --- |
| **Alauda Container Platform Log Storage for ClickHouse** | **Storage Path on Host** and **Metadata Storage Path**, or `spec.config.components.clickhouse.storagePolicy.*.localVolumeSpec.path` | `/var/cpaas/data/clickhouse` |
| **Alauda Container Platform Log Storage for Elasticsearch** | **Storage path on host**, or `spec.config.components.elasticsearch.hostpath` | `/var/cpaas/data/elasticsearch` |
| **Alauda Container Platform Log Collector** | **Storage Path**, or `spec.config.components.nevermore.storageSpec.dataDir` | `/var/cpaas/data/nevermore` |

Back the selected path with a persistent disk that survives node re-provisioning, and size that disk for the retention settings you plan to use. Set the path before you install, because several storage settings cannot be changed afterwards.
...
3. Refer to the following instructions to configure relevant parameters.
   ...
   | **Storage path on host** | Displayed when `LocalVolume` is used. Enter the absolute host path for Elasticsearch data. On Alauda OS nodes, use a path under `/var/cpaas`, for example `/var/cpaas/data/elasticsearch`. |
...
Before applying these scheduling rules, make sure your infra node planning and local storage placement are compatible. For the node planning considerations, see [Planning Infra Nodes for Logging Storage](./how_to/infra_nodes.mdx).

## Install Alauda Container Platform Log Storage for ClickHouse

### Console

If you plan to use S3 for ClickHouse log data storage or cold data storage, prepare the required S3 Secret before installation. For the complete procedure, including the Secret format, HTTPS CA certificate Secret, and usage modes, see [How to Use S3 Storage with ClickHouse](./how_to/clickhouse-s3-storage.mdx).
...
3. Refer to the following instructions to configure relevant parameters.
   ...
   | --- | --- |
   | **Size** | Choose `Single Node`, `Small Cluster`, or `Big Cluster` based on the target scale. |
   | **Storage Policy** | Controls how ClickHouse stores metadata and log data. Available values are **Default** and **Separation of storage and compute**. This setting cannot be changed after installation. |
   ...
   | **Storage Path on Host** | Displayed when **Storage Policy** is **Default** and **Storage Method** is `LocalVolume`. Default value: `/cpaas/data/clickhouse`. On Alauda OS nodes, change it to `/var/cpaas/data/clickhouse`. |
   | **Metadata Storage Path** | Displayed when **Metadata Storage Type** is `LocalVolume`. Default value: `/cpaas/data/clickhouse`. On Alauda OS nodes, change it to `/var/cpaas/data/clickhouse`. |
   ...
   | **Node Selectors** | Displayed in **Advanced Configuration**. Configure plugin-level node selector rules for the ClickHouse storage plugin workloads. |
   | **Node Tolerations** | Displayed in **Advanced Configuration**. Configure plugin-level toleration rules for the ClickHouse storage plugin workloads. |
   | **S3 for Log Data** | Displayed when **Storage Policy** is **Separation of storage and compute**. Select the prepared S3 Secret for log data storage. This setting cannot be changed after installation. |
   ...
   | **Cold and Hot Separate** | Enables tiered storage for ClickHouse. After it is enabled, you can configure an S3 location for cold data and set cold-data retention days by log type. This switch cannot be changed after installation. |
   | **S3 for Log Cold Data** | Displayed when **Cold and Hot Separate** is enabled. Select the prepared S3 Secret for cold data storage. This setting cannot be changed after installation. |
...
| `spec.config.components.nodeSelector` | Optional. Plugin-level node selector rules for the ClickHouse storage plugin workloads. This field uses the structured list format defined by the plugin API. |
| `spec.config.components.tolerations` | Optional. Plugin-level toleration rules for the ClickHouse storage plugin workloads. |
...
## Install Log Storage with External ClickHouse or OpenSearch
...
stringData:
  ...
  tls.ca: |-                                      # Required when the target uses HTTPS with a private CA, so the platform can verify the target
    -----BEGIN CERTIFICATE-----
...
Do not set `tls.insecure_skip_verify: "true"`. Provide `tls.ca` instead, so the platform can verify the target.
...
3. Refer to the following instructions to configure relevant parameters.
   ...
   | Parameter | Description |
   | --- | --- |
   | **Storage Cluster** | Select the cluster where Alauda Container Platform Log Storage has been installed. |
   | **Log types** | Click ***Select/Deselect*** to set the scope of log collection in the cluster. |
   | **Storage Path** | Configure the local working-data path for Log Collector. Use an absolute path. Default value: `/cpaas/data/nevermore`. On Alauda OS nodes, change it to `/var/cpaas/data/nevermore`. This setting cannot be changed after installation. |
...
spec:
  config:
    ...
    components:
      nevermore:
        storageSpec:
          dataDir: /cpaas/data/nevermore # On Alauda OS nodes: /var/cpaas/data/nevermore
    storage:
      type: ElasticSearch
  version: v4.1.0
  valuesOverride:
...
| `spec.config.components.nevermore.storageSpec.dataDir` | Storage path for the Log Collector local working data. Use an absolute path. This field cannot be changed after installation. |
