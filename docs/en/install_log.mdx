...
| Plugin | Field | Path on Alauda OS |
| --- | --- | --- |
| **Alauda Container Platform Log Storage with Clickhouse** | `spec.config.components.clickhouse.hostpath` | `/var/cpaas/data/clickhouse` |
| **Alauda Container Platform Log Storage with ElasticSearch** | `spec.config.components.elasticsearch.hostpath` | `/var/cpaas/data/elasticsearch` |
...
| `spec.config.components.elasticsearch.hostpath` | Data path for Elasticsearch. On Alauda OS nodes, use a path under `/var/cpaas`, for example `/var/cpaas/data/elasticsearch`. |
  ...
  config:
    components:
      ...
      clickhouse:
        ...
        k8sNodes:
          - xxx.xxx.xxx.xx
        hostpath: /cpaas/data/clickhouse # On Alauda OS nodes: /var/cpaas/data/clickhouse
        nodeReplicas: 1
        nodeStorageSize: 200
        type: single
      razor:
...
| `spec.config.components.clickhouse.hostpath` | Local path for ClickHouse data when using `LocalVolume`. On Alauda OS nodes, use a path under `/var/cpaas`, for example `/var/cpaas/data/clickhouse`. |
