...
1. Navigate to **App Store Management** > **Cluster Plugin** and select the `global` cluster.

2. In the **Plugins** tab, click the action button to the right of **Alauda Container Platform Log Essentials** > **Install**.
...
3. Refer to the following instructions to configure relevant parameters.
   ...
   | Parameter | Description |
   | --- | --- |
   | **Storage Policy** | Controls how ClickHouse stores metadata and log data.<br />Available values:<br />- **Default**: Use the standard local or StorageClass-based storage for ClickHouse data.<br />- **Separation of storage and compute**: Store ClickHouse metadata locally while storing log data in remote S3 storage.<br />This setting cannot be changed after deployment. |
   | **Storage Type** | Displayed when **Storage Policy** is **Default**. Available values: `LocalVolume` and `StorageClass`.<br />- `LocalVolume`: Store ClickHouse log data on selected nodes and host paths.<br />- `StorageClass`: Store ClickHouse log data through the selected storage class. |
   | **MetaData Storage Type** | Displayed when **Storage Policy** is **Separation of storage and compute**. Available values: `LocalVolume` and `StorageClass`.<br />- `LocalVolume`: Store ClickHouse metadata on selected nodes and host paths.<br />- `StorageClass`: Store ClickHouse metadata through the selected storage class. |
   | **Log Nodes** | Displayed when the selected storage type is `LocalVolume`.<br />- Under **Default**, select the nodes that store ClickHouse log data locally.<br />- Under **Separation of storage and compute**, select the nodes that store ClickHouse metadata locally. |
   | **Storage Path** | Displayed when **Storage Policy** is **Default** and **Storage Type** is `LocalVolume`. Default value: `/cpaas/data/clickhouse`.<br />Enter an absolute host path for storing ClickHouse log data. |
   | **MetaData Storage Path** | Displayed when **MetaData Storage Type** is `LocalVolume`. Default value: `/cpaas/data/clickhouse`.<br />Enter an absolute host path for storing ClickHouse metadata. |
   | **Log Instances** | Displayed when the selected storage type is `StorageClass`.<br />- Under **Default**, controls the number of ClickHouse instances created for data storage.<br />- Under **Separation of storage and compute**, controls the number of ClickHouse instances created for metadata storage. |
   | **StorageClass** | Displayed when **Storage Policy** is **Default** and **Storage Type** is `StorageClass`. Select the StorageClass used for ClickHouse log data. |
   ...
   | **Capacity** | Displayed when **Storage Policy** is **Default**. Capacity reserved for ClickHouse log data. |
   | **MetaData Capacity** | Capacity reserved for ClickHouse metadata. The recommended value is `20 Gi`. |
   | **S3 for Log Data** | Displayed when **Storage Policy** is **Separation of storage and compute**. Select a Secret in `cpaas-system` with label `cpaas.io/credentials: S3`. This Secret contains the S3 endpoint, bucket, and credentials for log data storage. This setting cannot be changed after deployment. |
   | **Retention Period** | Maximum retention time for logs, events, and audit data in the active ClickHouse storage. Data exceeding the retention period will be automatically cleaned up or moved according to the configured cold storage policy. |
   | **Cold and Hot Separation** | Enables tiered storage for ClickHouse. After it is enabled, you can configure an S3 location for cold data and set the cold data retention period for each log type. This switch cannot be changed after deployment. |
   | **S3 for Cold Data** | Displayed when **Cold and Hot Separation** is enabled. Select a Secret in `cpaas-system` with label `cpaas.io/credentials: S3` for cold data storage. This setting cannot be changed after deployment. |
   | **Retention Time for Cold Data** | Displayed when **Cold and Hot Separation** is enabled. Configure the retention period for cold data by log type. A value of `0` means that this type of log is not transferred to cold storage. This setting can be updated after deployment. |
...
- Upgrading the ClickHouse storage plugin does not allow you to switch the **Storage Policy** type or toggle **Cold and Hot Separation** on or off.
- After deployment, only the effective retention settings, such as hot or cold retention days, should be adjusted.
- To use this capability, upgrade **Alauda Container Platform Log Essentials** and **Alauda Container Platform Log Storage with Clickhouse** to compatible versions together.
